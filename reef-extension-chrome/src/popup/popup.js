...
var libSelectedIndex = -1;
var browserSelectedIndex = -1;
var bookmarkStorageMode = "reef";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const btnBookmarkTab = document.getElementById("btn-bookmark-tab");
  const browserSearchInput = document.getElementById("browser-search-input");
  const browserSearchClear = document.getElementById("browser-search-clear");
  const browserTools = document.getElementById("browser-tools");
  const browserSubTabs = document.getElementById("browser-sub-tabs");
  ...
  function switchToTabSearch(query) {
    currentMode = "tabs";
    modeTabs.querySelectorAll(".mode-tab").forEach((b) => {
  ...
  function renderTabResults() {
    ...
    tabResults.forEach((item, idx) => {
      ...
      card.addEventListener("click", () => {
        chrome.runtime.sendMessage({ type: "TAB_SWITCH", tabId: item.tab.id, windowId: activeTabInfo?.windowId });
        window.close();
      });
      card.addEventListener("mouseenter", () => {
  ...
  async function loadBrowserPanel() {
    ...
    sessionsCache = sessions["reef:sessions"] || [];
  ...
  function applySelection(panel, idx, scroll) {
    let container;
