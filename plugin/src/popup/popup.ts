 ...
 * Three modes: Page (existing single-page search), Tabs (cross-tab search),
 * Library (bookmarks / snippets / notes / recents).
 */

import type { IndexRecord } from '../../src/types.js';
import type { Bookmark, Snippet, PageNote, RecentPage } from '../storage.js';

type Mode = 'page' | 'tabs' | 'library';
type LibTab = 'bookmarks' | 'snippets' | 'notes' | 'recents';

let activeTabId: number | null = null;
let currentMode: Mode = 'page';

// Page state
let pageFilter: string = 'all';
...
// Library state
let libFilter: LibTab = 'bookmarks';
let libQuery: string = '';
let bookmarksCache: Bookmark[] = [];
let snippetsCache: Snippet[] = [];
let notesCache: PageNote[] = [];
let recentsCache: RecentPage[] = [];
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const panels = document.querySelectorAll('.mode-panel') as NodeListOf<HTMLElement>;
  ...
  function renderPageResults() {
    ...
    if (pageFilter !== 'all') filtered = pageResults.filter(r => r.type === pageFilter);
    ...
    filtered.forEach(record => {
      ...
      runBtn.textContent =
        record.type === 'action' || record.type === 'link'
          ? (record.destructive ? '⚠️ Run (Confirm)' : 'Execute')
          : record.type === 'field' ? 'Fill Field' : 'View';
      runBtn.addEventListener('click', (e) => { e.stopPropagation(); executePageAction(record); });
  ...
  function executePageAction(record: IndexRecord) {
    if (!activeTabId) return;
  ...
  function bookmarkRecord(record: IndexRecord) {
    chrome.runtime.sendMessage({
  ...
  // ─── TABS MODE ──────────────────────────────────────
  let tabDebounce: number | null = null;
  tabSearchInput.addEventListener('input', () => {
    ...
    tabDebounce = window.setTimeout(runTabSearch, 150) as unknown as number;
  });
  ...
  function runTabSearch() {
    ...
    chrome.runtime.sendMessage({ type: 'TAB_SEARCH', query: tabQuery, limit: 30 }, (res: any) => {
      if (!res?.success) {
  ...
  function renderTabResults() {
    ...
    tabResults.forEach(item => {
      ...
      if (item.matchedRecords?.length) {
        const snippet = document.createElement('div');
        snippet.className = 'card-snippet';
        snippet.textContent = item.matchedRecords.map((r: any) => r.headingText).filter(Boolean).join(' · ');
        card.appendChild(snippet);
      }
  ...
  // ─── LIBRARY MODE ───────────────────────────────────
  let libDebounce: number | null = null;
  libSearchInput.addEventListener('input', () => {
    ...
    libDebounce = window.setTimeout(renderLibrary, 120) as unknown as number;
  });
  ...
  libTabs.addEventListener('click', (e) => {
    ...
    libFilter = (t.dataset.lib as LibTab) || 'bookmarks';
    renderLibrary();
  });
  ...
  async function loadLibrary() {
    ...
    const req = (type: string) => new Promise<any>((resolve) => {
      chrome.runtime.sendMessage({ type }, (res: any) => resolve(res));
    });
    ...
    bookmarksCache = (bm?.items || []) as Bookmark[];
    snippetsCache = (sn?.items || []) as Snippet[];
    notesCache = (nt?.items || []) as PageNote[];
    recentsCache = (rc?.items || []) as RecentPage[];
    renderLibrary();
  }
  ...
  function filtered<T extends { tags?: string[] }>(items: T[], hayMaker: (i: T) => string): T[] {
    let out = items;
    if (libQuery) {
  ...
  function renderBookmarks() {
    const items = filtered(bookmarksCache, (b) =>
      [b.title, b.selectedText, b.note, (b.tags || []).join(' '), b.url].filter(Boolean).join(' '));
    ...
    items.forEach((b) => {
      ...
      meta.textContent = `${shortHost(b.url)} · ${formatDate(b.createdAt)}`;
  ...
  function renderSnippets() {
    const items = filtered(snippetsCache, (s) =>
      [s.title, s.text, (s.tags || []).join(' '), s.source?.title || ''].filter(Boolean).join(' '));
    ...
    items.forEach((s) => {
      ...
      if (s.source) {
        const meta = document.createElement('div');
        meta.className = 'card-meta';
        meta.textContent = `from ${s.source.title || shortHost(s.source.url)} · ${formatDate(s.createdAt)}`;
        card.appendChild(meta);
      }
  ...
  function renderNotes() {
    ...
    items.forEach((n) => {
      ...
      const meta = document.createElement('div');
      meta.className = 'card-meta';
      meta.textContent = `${shortHost(n.url)} · ${formatDate(n.updatedAt)}`;
      card.appendChild(meta);
      const actions = document.createElement('div');
      actions.className = 'card-actions';
      const openBtn = document.createElement('button');
  ...
  function renderRecents() {
    ...
    items.forEach((p) => {
      ...
      const meta = document.createElement('div');
      meta.className = 'card-meta';
      meta.textContent = `${shortHost(p.url)} · ${formatDate(p.visitedAt)}`;
      card.appendChild(meta);
      const actions = document.createElement('div');
      actions.className = 'card-actions';
      const openBtn = document.createElement('button');
  ...
  function deleteBookmark(id: string) {
    ...
    chrome.runtime.sendMessage({ type: 'LIBRARY_BOOKMARK_DELETE', id }, (res: any) => {
      if (res?.success) {
  ...
  function editSnippet(s: Snippet) {
    ...
    chrome.runtime.sendMessage(
      ...
      (res: any) => {
        if (res?.success) {
          s.title = title; s.text = text; s.tags = tags;
  ...
  function deleteSnippet(id: string) {
    ...
    chrome.runtime.sendMessage({ type: 'LIBRARY_SNIPPET_DELETE', id }, (res: any) => {
      if (res?.success) {
  ...
  function deleteNote(url: string) {
    if (!confirm('Delete this note?')) return;
    chrome.runtime.sendMessage({ type: 'LIBRARY_NOTE_DELETE', url }, (res: any) => {
      if (res?.success) {
  ...
  function flashStats(msg: string) {
