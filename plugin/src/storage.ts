...
export async function updateBookmark(id: string, patch: Partial<Omit<Bookmark, 'id'>>): Promise<Bookmark | null> {
  const all = await getArray<Bookmark>(KEY.bookmarks);
...
export async function updateSnippet(id: string, patch: Partial<Omit<Snippet, 'id'>>): Promise<Snippet | null> {
  const all = await getArray<Snippet>(KEY.snippets);
...
export async function recordRecent(page: Omit<RecentPage, 'visitedAt'>): Promise<void> {
  ...
  const next = [{ ...page, visitedAt: Date.now() }, ...all.filter((p) => p.url !== page.url)].slice(
    0,
    RECENT_MAX
  );
