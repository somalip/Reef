...
function createDefaultTokenizePipeline() {
    return [
        (token) => {
...
function tokenizeText(text, pipeline) {
    if (!text)
        return [];
...
export function addToIndex(index, records, tokenizePipeline) {
...
export function getAllSections(index) {
...
export function findClosestWord(query, index, maxDistance = 2) {
    const term = query.trim().toLowerCase();
    if (!term || term.length < 2)
        return null;
    let closest = null;
    const checkWord = (word) => {
        if (!word || Math.abs(word.length - term.length) > maxDistance)
            return;
...
export function parseExtendedQuery(query) {
...
export function searchSections(query, index, limitOrOptions) {
    let limit = 8;
    let options;
...
export function removeFromIndex(index, id) {
    ...
    if (headingIds) {
        ...
        if (idx !== -1)
            headingIds.splice(idx, 1);
        if (headingIds.length === 0)
            index.headingIds.delete(headingLower);
    }
    for (let i = 2; i <= headingLower.length; i++) {
        const prefix = headingLower.slice(0, i);
        ...
        if (prefixRecords) {
            ...
            if (idx !== -1)
                prefixRecords.splice(idx, 1);
            if (prefixRecords.length === 0)
                index.headingIndex.delete(prefix);
        }
    }
...
export function suggest(query, index, limit = 10) {
...
export function trackQuery(index, query) {
    const q = query.trim();
    if (!q)
        return;
...
export function getPopularQueries(index, n = 5) {
