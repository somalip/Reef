...
export function extractHeadingId(fullMatch, text) {
...
export function hasExplicitId(fullMatch) {
    return /\bid=["'][^"']+["']/i.test(fullMatch);
}
export function findParentSectionId(html, headingMatchEnd) {
...
export function extractSections(html, url) {
...
export function extractActionName(element) {
...
export function isDestructiveAction(label) {
...
export function extractActions(html, url, excludeSelectors) {
    const actions = [];
...
export function extractFields(html, url) {
    const fields = [];
    const doc = new DOMParser().parseFromString(html, 'text/html');
...
export function extractHiddenContent(doc) {
    doc.querySelectorAll('details').forEach(d => {
        if (!d.hasAttribute('open'))
            d.setAttribute('data-reef-was-closed', 'true');
        d.setAttribute('open', '');
    });
...
export function extractLinks(html, url) {
    const links = [];
    const doc = new DOMParser().parseFromString(html, 'text/html');
    const anchors = Array.from(doc.querySelectorAll('a[href]'));
    for (const anchor of anchors) {
        if (anchor.hasAttribute('rel') && anchor.getAttribute('rel')?.toLowerCase().includes('nofollow'))
            continue;
...
export function extractFiles(html, url, extensions) {
    const files = [];
    const doc = new DOMParser().parseFromString(html, 'text/html');
...
export function extractMedia(html, url) {
    const media = [];
    const doc = new DOMParser().parseFromString(html, 'text/html');
...
export function extractStructuredData(html, url) {
    ...
    for (const script of jsonLdScripts) {
        try {
            ...
            if (Array.isArray(data) ? data.some((item) => item['@type'] === 'FAQPage') :
                data['@type'] === 'FAQPage') {
                const faqItems = Array.isArray(data) ?
                    data.flatMap((item) => item.mainEntity || []) :
                    data.mainEntity || [];
